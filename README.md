...
完成了基础的模块组装，还需要后面继续更新。

**核心定义：** 我们要做什么？  
跨模态、开集（Open-set / Unseen classes）、单样本（One-shot）、特征配准（Registration）。

**核心思路：** 寻找 “结构共现” (Structural Co-occurrence)。

> **Note:** 我们不做图像生成（GAN），我们做图像配准（Registration）。
...
* **输出：** 两个高维几何特征张量 $F_{rgb}$ 和 $F_{ir}$ (维度 $[1, 256, H/8, W/8]$)。
...
* **输入：** $F_{rgb}$ 和 $F_{ir}$ 。
* **操作流程：**
    1.  **L2 归一化：** 对 256 个通道进行单位化，消除特征强度的干扰，只比较“方向”（即结构形状）。
    2.  **展平 (Flatten)：** 将空间维度 $(H/8, W/8)$ 拉平为 $N$ 。
    3.  **点积运算：** 计算 $F_{rgb}^T \times F_{ir}$ 。
* **输出：** 相关性矩阵 (Correlation Matrix)，维度 $[N, N]$ 。
...
* **操作流程：**
    1.  **特征匹配 (Soft-Argmax)：** 从第三步的相关性矩阵中，解析出 $K$ 对置信度最高的匹配点对 $(x_{rgb}, y_{rgb}) \leftrightarrow (x_{ir}, y_{ir})$ 。
    2.  **几何验证 (RANSAC)：**
        * 从 $K$ 对点中随机抽取子集。
        * 计算仿射变换矩阵 (Affine Matrix, $2 \times 3$)。
        * 验证剩余点是否符合该矩阵，循环迭代找出最优解。
    3.  **标签变换 (Warping)：** 利用算出的最终矩阵，将 $M_{rgb}$ (可见光标签) 进行仿射变换。
